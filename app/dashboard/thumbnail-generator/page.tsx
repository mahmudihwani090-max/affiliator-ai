...
import { checkImageJobStatus, generateImageToImage, generateTextToImage, upscaleImage } from "@/lib/client/generation-api"
...
import { buildThumbnailGeneratorPrompt, type ThumbnailGeneratorConfig } from "@/lib/thumbnail-generator"
import { CheckCircle2, Copy, Download, Eye, Image as ImageIcon, Loader2, PanelTop, RefreshCw, Sparkles, X, ZoomIn } from "lucide-react"
...
type GeneratedThumbnail = {
    ...
    sourceConfig: ThumbnailGeneratorConfig
    mediaGenerationId?: string
    isUpscaled?: boolean
}
...
type ThumbnailPreset = {
    name: string
    description: string
    values: Partial<ThumbnailGeneratorConfig>
}

const visualStyleOptions = ["High contrast YouTube", "Clean modern", "Bold dramatic", "Reaction face thumbnail", "Premium educational"]
const emotionOptions = ["Curious", "Shocked", "Excited", "Confident", "Urgent"]
const colorMoodOptions = ["Bright saturated", "Dark dramatic", "Warm premium", "Blue tech", "Red urgency"]
const thumbnailPresets: ThumbnailPreset[] = [
    {
        name: "Education",
        description: "Untuk video tutorial dan explainer yang butuh kredibilitas tinggi.",
        values: { visualStyle: "Premium educational", emotion: "Confident", colorMood: "Blue tech", aspectRatio: "landscape" },
    },
    {
        name: "Viral Reaction",
        description: "Untuk hook heboh, surprise, atau reveal yang agresif.",
        values: { visualStyle: "Reaction face thumbnail", emotion: "Shocked", colorMood: "Red urgency", aspectRatio: "landscape" },
    },
    {
        name: "Product Promo",
        description: "Untuk thumbnail promo produk atau launch campaign.",
        values: { visualStyle: "Bold dramatic", emotion: "Excited", colorMood: "Warm premium", aspectRatio: "landscape" },
    },
    {
        name: "Short Cover",
        description: "Untuk cover vertical short atau reels teaser.",
        values: { visualStyle: "Clean modern", emotion: "Curious", colorMood: "Bright saturated", aspectRatio: "portrait" },
    },
]

const defaultConfig: ThumbnailGeneratorConfig = {
    contentTopic: "",
    hookAngle: "",
    focalSubject: "",
    visualStyle: "High contrast YouTube",
    emotion: "Curious",
    colorMood: "Bright saturated",
...
export default function ThumbnailGeneratorPage() {
    const [config, setConfig] = useState<ThumbnailGeneratorConfig>(defaultConfig)
    ...
    const updateConfig = <K extends keyof ThumbnailGeneratorConfig>(key: K, value: ThumbnailGeneratorConfig[K]) => setConfig((current) => ({ ...current, [key]: value }))

    const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        const files = Array.from(event.target.files ?? []).slice(0, 2 - referenceImages.length)
        try {
            ...
            setReferenceImages((current) => [...current, ...images].slice(0, 2))
        } catch (error) {
    ...
    const createImage = async (nextConfig: ThumbnailGeneratorConfig, nextReferences: string[]) => {
        if (!nextConfig.contentTopic.trim() || !nextConfig.hookAngle.trim() || !nextConfig.focalSubject.trim()) {
            throw new Error("Topic, hook angle, dan focal subject wajib diisi")
        }

        const prompt = buildThumbnailGeneratorPrompt(nextConfig)
        ...
        if (!result.success) throw new Error(result.error || "Gagal generate thumbnail")
    ...
    const handleRegenerate = async (image: GeneratedThumbnail) => {
        setIsGenerating(true)
        try {
            ...
            toast.success("Variasi thumbnail baru berhasil dibuat")
        } catch (error) {
            toast.error(toUserFacingGenerationError(error instanceof Error ? error.message : undefined, "image"))
        } finally { setIsGenerating(false) }
    }

    const handleDuplicateAsDraft = (image: GeneratedThumbnail) => {
        setConfig({ ...image.sourceConfig })
        setReferenceImages([...image.referenceImages])
        toast.success("Draft thumbnail dimuat kembali ke form")
    }

    const handleUpscale = async (image: GeneratedThumbnail) => {
        ...
        try {
            ...
            if (!result.success) throw new Error(result.error || "Gagal upscale thumbnail")
            ...
            toast.success("Thumbnail berhasil di-upscale ke 2K")
        } catch (error) {
                ...
                <Card className="rounded-[2rem] border-gray-200 bg-white py-0 text-foreground shadow-lg dark:border-white/8 dark:bg-[#111111] dark:shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
                    <div className="space-y-6 p-5 md:p-6">
                        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                            <div>
                                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-emerald-600 dark:text-emerald-300"><PanelTop className="h-3.5 w-3.5" /> Thumbnail Generator</div>
                                <h1 className="text-2xl font-black tracking-tight md:text-3xl">Thumbnail Generator</h1>
                                <p className="mt-2 max-w-2xl text-sm text-zinc-500 dark:text-zinc-400">Generate thumbnail image untuk YouTube, cover reels, atau social content dengan preset cepat, lalu refine dengan regenerate, duplicate draft, preview, dan upscale.</p>
                            </div>
                            <div className="grid grid-cols-2 gap-2 text-xs text-zinc-600 dark:text-zinc-300 md:min-w-[280px]">
                                <div className="rounded-2xl border border-gray-200 bg-gray-100 px-4 py-3 dark:border-white/10 dark:bg-white/5"><p className="text-zinc-500">Preset</p><p className="mt-1 font-semibold text-foreground">4 quick-start style</p></div>
                                <div className="rounded-2xl border border-gray-200 bg-gray-100 px-4 py-3 dark:border-white/10 dark:bg-white/5"><p className="text-zinc-500">Output</p><p className="mt-1 font-semibold text-foreground">Landscape / portrait</p></div>
                            </div>
                        </div>

                        <div className="space-y-3 rounded-[1.5rem] border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-[#0c0c0c]">
                            <div>
                                <h2 className="text-sm font-semibold text-foreground">Preset Thumbnail</h2>
                                <p className="mt-1 text-sm text-zinc-500">Pilih preset untuk set arah visual lebih cepat, lalu sesuaikan topic, hook, dan subject.</p>
                            </div>
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{thumbnailPresets.map((preset) => <button key={preset.name} type="button" onClick={() => applyPreset(preset)} className="rounded-2xl border border-gray-200 bg-gray-100 p-4 text-left transition hover:border-emerald-400/40 hover:bg-emerald-400/10 dark:border-white/10 dark:bg-white/5"><p className="text-sm font-semibold text-foreground">{preset.name}</p><p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-zinc-400">{preset.description}</p></button>)}</div>
                        </div>

                        <div className="grid gap-6 md:grid-cols-2">
                            <div><FieldLabel>Content Topic</FieldLabel><DarkInput value={config.contentTopic} onChange={(event) => updateConfig("contentTopic", event.target.value)} placeholder="Contoh: cara cepat naikkan conversion" /></div>
                            <div><FieldLabel>Focal Subject</FieldLabel><DarkInput value={config.focalSubject} onChange={(event) => updateConfig("focalSubject", event.target.value)} placeholder="Contoh: creator pointing to graph" /></div>
                            <div className="md:col-span-2"><FieldLabel>Hook Angle</FieldLabel><DarkTextarea value={config.hookAngle} onChange={(event) => updateConfig("hookAngle", event.target.value)} placeholder="Contoh: rahasia yang jarang dibahas tapi paling penting" /></div>
                            <div><FieldLabel>Visual Style</FieldLabel><DarkSelect value={config.visualStyle} onValueChange={(value) => updateConfig("visualStyle", value)} options={visualStyleOptions} /></div>
                            <div><FieldLabel>Emotion</FieldLabel><DarkSelect value={config.emotion} onValueChange={(value) => updateConfig("emotion", value)} options={emotionOptions} /></div>
                            <div><FieldLabel>Color Mood</FieldLabel><DarkSelect value={config.colorMood} onValueChange={(value) => updateConfig("colorMood", value)} options={colorMoodOptions} /></div>
                            <div><FieldLabel>Aspect Ratio</FieldLabel><DarkSelect value={config.aspectRatio === "portrait" ? "9:16" : "16:9"} onValueChange={(value) => updateConfig("aspectRatio", value === "9:16" ? "portrait" : "landscape")} options={["16:9", "9:16"]} /></div>
                            <div className="md:col-span-2"><FieldLabel>Optional Reference Images (Max 2)</FieldLabel><Button type="button" variant="outline" className="h-14 w-full rounded-2xl border-gray-300 bg-transparent text-foreground hover:bg-gray-100 dark:border-white/10 dark:text-white dark:hover:bg-white/5" onClick={() => inputRef.current?.click()}><ImageIcon className="mr-2 h-4 w-4" />Upload References</Button></div>
                        </div>

                        {referenceImages.length > 0 ? <div className="grid gap-3 sm:grid-cols-2">{referenceImages.map((image, index) => <div key={`${image.slice(0, 20)}-${index}`} className="relative overflow-hidden rounded-2xl border border-gray-200 bg-gray-100 dark:border-white/10 dark:bg-black"><Image src={image} alt={`Thumbnail ref ${index + 1}`} width={280} height={280} unoptimized className="aspect-square w-full object-cover" /><button type="button" onClick={() => setReferenceImages((current) => current.filter((_, currentIndex) => currentIndex !== index))} className="absolute right-2 top-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-black/70 text-white"><X className="h-4 w-4" /></button></div>)}</div> : null}
                        ...
                        <div className="flex flex-col gap-3 border-t border-gray-200 pt-4 md:flex-row md:items-center md:justify-between dark:border-white/10">
                            <p className="text-sm text-zinc-500">Result card menyimpan preset yang dipakai, draft config, dan reference supaya lebih cepat bikin batch variasi thumbnail.</p>
                            <Button onClick={handleGenerate} disabled={isGenerating} className="h-11 rounded-xl bg-emerald-500 px-5 text-sm font-semibold text-black hover:bg-emerald-400">{isGenerating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Generating...</> : <><Sparkles className="mr-2 h-4 w-4" />Generate Thumbnail</>}</Button>
                        </div>
                    </div>
                </Card>
